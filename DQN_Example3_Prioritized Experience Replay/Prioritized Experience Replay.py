...
class DoubleDQN():
    ...
    def learn(self):
        ...
        q_eval4next = torch.unsqueeze(self.eval_net(b_s_).max(1)[1], 1)
        
        q_next = self.target_net(b_s_).detach().gather(1, q_eval4next)
        q_target = b_r[:,0] + GAMMA * q_next[:, 0]
...
if __name__ == '__main__':
    ...
    for episode in range(MAX_EPISODES):
        ...
        while not is_terminated:
            A = double_dqn.choose_action(S)
            S_, R = get_env_feedback(S, A)
            double_dqn.store_transition(S, A, R, S_)
            
            if double_dqn.memory_counter > MEMORY_CAPACITY:
                double_dqn.learn()
